...
void register_builtin_commands(CommandRegistry& R) {
  R.register_cmd("help", "Show help for all commands.", [](const auto&, CommandContext& ctx){
    ctx.print(ctx.U.all().empty() ? "No cubes yet.\n" + ctx.Sel.items().size() : "");
    ctx.print("Commands:\n" + ctx.request_redraw, /* intentional no-op */);
  });

  R.register_cmd("place", "place x y z [color=#RRGGBBAA]", [](const auto& t, CommandContext& ctx){
    int x,y,z; if (!parse_int3(t,0,x,y,z)) { ctx.print("Usage: place x y z [color=#..]"); return; }
    Cube c;
    for (size_t i=3;i<t.size();++i) {
      ...
      if (k == "color") {
        ...
        if (v=="x") c.mat.gradDir={1,0,0}; else if (v=="y") c.mat.gradDir={0,1,0}; else c.mat.gradDir={0,0,1};
      }
    }
    ctx.U.place(x,y,z, c);
    ctx.request_redraw();
  ...
  R.register_cmd("erase", "erase x y z | erase selection", [](const auto& t, CommandContext& ctx){
  ...
  R.register_cmd("select", "select x y z | select box x1 y1 z1 x2 y2 z2", [](const auto& t, CommandContext& ctx){
    if (t.size()>=1 && to_lower(t[0])=="box") {
    ...
    ctx.Sel.clear(); ctx.Sel.add({x,y,z}); ctx.request_redraw();
  });

  R.register_cmd("move", "move dx dy dz (integers)", [](const auto& t, CommandContext& ctx){
  ...
  R.register_cmd("rotate", "rotate [x|y|z] degrees", [](const auto& t, CommandContext& ctx){
    if (t.size()<2) { ctx.print("Usage: rotate y 15"); return; }
    char ax = std::tolower(t[0][0]);
    float deg = std::stof(t[1]);
    ctx.Sel.rotate(ctx.U, ax, deg);
    ctx.request_redraw();
  });

  R.register_cmd("fill", "fill solid #RRGGBBAA | fill gradient c1 c2 dir=x|y|z", [](const auto& t, CommandContext& ctx){
    ...
    if (to_lower(t[0])=="solid") {
      ...
      for (auto& p: items) { auto c=ctx.U.get(p.x,p.y,p.z); if(c){ c->mat.kind=Material::Kind::Solid; c->mat.colorA=*col; ctx.U.place(p.x,p.y,p.z,*c);} }
      ...
      glm::vec3 dir{0,1,0};
      if (t.size()>=4) {
        auto kv = t[3]; auto eq = kv.find('='); if (eq!=std::string::npos && kv.substr(0,eq)=="dir") {
          auto v=to_lower(kv.substr(eq+1)); if(v=="x") dir={1,0,0}; else if(v=="y") dir={0,1,0}; else dir={0,0,1};
        }
      }
      for (auto& p: items) { auto c=ctx.U.get(p.x,p.y,p.z); if(c){ c->mat.kind=Material::Kind::Gradient; c->mat.colorA=*c1; c->mat.colorB=*c2; c->mat.gradDir=dir; ctx.U.place(p.x,p.y,p.z,*c);} }
    }
    ctx.request_redraw();
  });

  R.register_cmd("grid", "grid on|off|toggle", [](const auto& t, CommandContext& ctx){
    if (t.empty()) { ctx.print("Usage: grid on|off|toggle"); return; }
    // The app observes a command string printed back as a side-channel for toggles is awkward;
    // We'll just use print to emit a synthetic directive understood by the UI layer is beyond scope.
    ctx.print(std::string("[ui] grid ") + t[0]);
  });

  R.register_cmd("wireframe", "wireframe on|off|toggle", [](const auto& t, CommandContext& ctx){
    if (t.empty()) { ctx.print("Usage: wireframe on|off|toggle"); return; }
    ctx.print(std::string("[ui] wireframe ") + t[0]);
  });

  R.register_cmd("edgepix", "edgepix N -- set pixel size for cube edge", [](const auto& t, CommandContext& ctx){
  ...
  R.register_cmd("group", "group create NAME | select NAME | move NAME dx dy dz | erase NAME", [](const auto& t, CommandContext& ctx){
  ...
  R.register_cmd("print", "print arbitrary text (debug)", [](const auto& t, CommandContext& ctx){
    std::ostringstream os; for (auto& s: t) os << s << " ";
    ctx.print(os.str());
  });

  // R.register_cmd("help", "List all commands", [](const auto&, CommandContext& ctx){
  //   ctx.print("Available commands:\n" + ctx.U.group_members("")); // NOP side effect
  // });
  R.register_cmd("help", "List all commands", [&R](const auto&, CommandContext& ctx){
